/* Собрано tools/inline_mock.py из design/build/library.html — руками не править.
...
/* ---- данные каталога ----------------------------------------------------
   Каталог показывается на языке ЧИТАТЕЛЯ целиком: названия книг, авторы и
   подписи полок. Раньше всё это было зашито по-русски, и на любой паре без
   русского полки оставались русскими.

   Приходит из PairPayload через __P: __P.catalog — устройство полок (один раз,
   языка не знает), __P.catText — текст этой локали, __P.ui — подписи. Набор по
   умолчанию английский и нужен только затем, чтобы мок оставался рабочей
   отдельной страницей; на лендинге он не берётся никогда.  */
...
// Полка для отрисовки: подписи по ключу строки, книги — по местам из устройства.
function shelf(key){
...
let s='';
// шапка (Профиль/Поиск/Загрузите книгу) — фиксирована над скроллом, чтобы не уезжала при прокрутке
...
// табы (Каталог/Все книги/Избранное) — тоже в фиксированной шапке
...
/* interactive top tabs */
/* catalog grid redrawn as clean SVG (source catalog.png has a misaligned middle row) */
...
// ==== мок внутри лендинга ====
// Признаком служило «я во фрейме». После встраивания компонентом фрейма нет —
// self === top, — и весь блок вместе с анимацией молча не запускался бы.
// Второй признак: ?demo в параметрах, которые смонтированный компонент получает
// из __P.search. Тот же приём, что у ридера.
// id контейнера, в который смонтирован компонент; у отдельной страницы его нет
const MOUNT_ID = (typeof __R !== 'undefined' && __R && __R.id) || '';
const IFRAME = window.self !== window.top;
const EMBED = IFRAME || /[?&]demo\b/.test(location.search);
if (EMBED) {
  // Подложку отдельной страницы убирает ТОЛЬКО фрейм. У смонтированного
  // компонента document.body — это body лендинга, и эта строка перекрасила бы
  // всю страницу: так у ридера появлялась серая полоса во весь экран.
  if (IFRAME) {
  ...
  // hero-демо библиотеки: скролл к «Золотая классика» → палец тапает обложку над «Читать» → openBook
  (function(){
    ...
    // задаются в координатах макета, до него. Пока мок жил во фрейме, предков с
    // transform не было и разница не мешала; в составе страницы экран ужат, и
    // палец уезжал вверх — тапал выше обложки. Отдельной страницей масштаб 1,
    // поэтому поведение мока не меняется. То же самое чинилось у ридера.
    ...
    async function run(){
      do {
        ...
        // Мотаем к зелёной врезке «5 минут в день», а не в самый низ: раньше
        // здесь стояло scrollHeight - clientHeight, то есть конец списка, и
        // заголовок врезки уезжал за верхний край.
        if (scroll) {
        ...
        await sleep(300);
        // Раньше лендинг отличал hero-библиотеку от карточки «цикла» по
        // e.source: у фреймов это разные contentWindow. У встроенных
        // компонентов источник один и тот же — window самой страницы, — поэтому
        // компонент называет себя сам.
