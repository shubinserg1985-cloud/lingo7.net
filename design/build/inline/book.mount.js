/* Собрано tools/inline_mock.py из design/build/book.html — руками не править.
...
// ---- данные книги -------------------------------------------------------
// Карточка показывается на языке ЧИТАТЕЛЯ целиком. Приходит из PairPayload:
// __P.catalog — устройство каталога (языка не знает), __P.catText — текст этой
// локали, __P.ui — подписи. Набор по умолчанию английский и взят из базы, а не
// написан заново; нужен только затем, чтобы мок оставался рабочей отдельной
// страницей.
...
// Карусель листает книги по МЕСТУ в ней (0…18), а не по id записи: место —
// свойство экрана, id — свойство каталога, и путать их нельзя.
function bookAt(pos) {
...
// Подписи по метке — ДО значений книги: иначе цикл затрёт жанр словом «Жанр».
...
// embed-режим (в карточке блока) — убрать standalone-подложку, заполнить кадр
// Признаком «я на лендинге» было «я во фрейме». У смонтированного компонента
// self === top; второй признак — ?demo из __P.search, как у ридера и каталога.
const IFRAME = window.self !== window.top;
if (IFRAME || /[?&]demo\b/.test(location.search)) {
  // body трогает только фрейм: у компонента это body самого лендинга
  if (IFRAME) {
